---
title: "avstrat-workflow-examples"
output: quarto::html
vignette: >
  %\VignetteIndexEntry{avstrat-workflow-examples}
...
focused_section <- ggstrat(df = mydata, 
                                stratsection_name = "21LSHD02", 
                                ylim = c(160,200))
grainsize_increasing + focused_section
```

Adding labels to the layers is often desired, but usually alongside one of the section visualizations from above. We can do this with `ggstrat_label()`, with default mapping to "SampleID", but any variable can be mapped to this. As in the examples above, `patchwork` provides a nice interface to combine plots.
